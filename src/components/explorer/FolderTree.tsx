...
const FolderNode: React.FC<FolderNodeProps> = ({
  ...
  const isAnyFolderDragging = !!draggingFolderId;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setIsOpen(true), 220);
  ...
  const handleNodeClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    onSelect(folder.id);
    if (hasSubFolders) setIsOpen(!isExpanded);
  };
  ...
  return (
    <motion.div className="select-none relative">
      <motion.div 
        ...
        whileTap={{ scale: 0.99 }}
        className={cn(
          "flex items-center gap-1.5 py-2.5 px-2 min-h-11 rounded-xl cursor-pointer transition-all duration-200 group relative border mb-1 will-change-transform",
...
export const FolderTree: React.FC<{
  ...
  const fetchFolders = async () => {
    try {
      setFolders(await explorerService.getFolders());
    } catch {
      // Keep the last known tree if refresh fails.
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-0.5 pb-32 overflow-x-hidden pr-1 min-h-[50vh] transition-all rounded-2xl relative">
      ...
      >
        ...
        <span className="text-[13px] uppercase tracking-widest font-bold">Workspace Root</span>
